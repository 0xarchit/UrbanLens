...
console.log('       Backend:  https://cityissue-back.0xarchit.is-a.dev\n');
...
try {
  ...
  execSync('docker compose down', { cwd: path.join(__dirname, 'infra', 'cloudflared'), stdio: 'ignore' });
} catch (e) {
}
...
function cleanup() {
  ...
  try {
    ...
    execSync('docker compose down', { cwd: absCloudflaredDir, stdio: 'inherit' });
